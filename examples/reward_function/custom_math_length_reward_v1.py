...
def laser_d_length_reward(gen_len, target_L, delta=0.1):
...
def compute_score(reward_inputs: List[Dict[str, Any]], format_weight: float = 0.1, length_weight: float = 0.1) -> List[Dict[str, float]]:
    ...
    for reward_input in reward_inputs:
        ...
        laser_d_length_score = laser_d_length_reward(gen_len=reward_input["response_length"], target_L = reward_input.get("target_length", DEFAULT_TARGET_LENGTH), delta=0.1)
        ...
        scores.append(
            {
                "overall": (1 - format_weight - length_weight) * accuracy_score + format_weight * format_score + length_weight * laser_d_length_score,
                "format": format_score,
