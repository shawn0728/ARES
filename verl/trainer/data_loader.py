...
def create_bootstrap_dataloader(config: DataConfig, tokenizer: PreTrainedTokenizer, processor: Optional[ProcessorMixin]) -> None:
    train_dataset = RLHFDataset(
        data_path=config.train_files,
        tokenizer=tokenizer,
        processor=processor,
    ...
    assert len(train_dataloader) >= 1
    assert len(val_dataloader) >= 1
    print(f"Size of bootstrap train dataloader with batch size 1024: {len(train_dataloader)}")
    return train_dataloader, val_dataloader


def create_filtered_train_dataloader(
    config,
    base_dataset,
    skip_gid_set,
):
    filtered_dataset = FiltereRLHFdDataset(base_dataset, skip_gid_set)
    # import pdb;pdb.set_trace()
    if config.shuffle:
        generator = torch.Generator()
        generator.manual_seed(config.seed)
        sampler = RandomSampler(filtered_dataset, generator=generator)
    else:
        sampler = SequentialSampler(filtered_dataset)
