    ...
    for logprobs_per_token in logprobs_per_token_list:
        ...
        for i, tok in enumerate(token_strs):
            if tok in reasoning_token_set:
                # check if there are enough following tokens to form a complete window
                ...
                if remaining_length >= window_size:
                    ...
                    # continuous soft cost: higher is more expensive
...
def get_window_average_entropy_percentile_threshold_list(
    ...
    """
    calculate the percentile threshold based on the window average entropy
    1. calculate the average entropy of the following window_size tokens for each token
    2. collect all window average entropies, calculate percentile
    3. only calculate the window average entropy when there are enough remaining tokens
    """
    ...
    for sample_idx, logprobs_per_token in enumerate(logprobs_per_token_list):
        ...
        # calculate the entropy of each token
        token_entropies = []
        for logprob_dict in logprobs_per_token:
            log_probs = np.array([logprob.logprob for logprob in logprob_dict.values()], dtype=np.float32)
        ...
        # calculate the average entropy of the following window for each token
        for i in range(len(token_entropies)):
            ...
            if remaining_length >= window_size:
                ...
                # remaining length is not enough, add the previous average value to keep the batch length consistent
                if window_average_entropies:
                    window_average_entropies.append(np.mean(window_average_entropies))
                else:
                    # if there is no window average entropy yet, use the entropy of the current token
                    window_average_entropies.append(token_entropies[i])
        
        # calculate the percentile of the window average entropy
        if window_average_entropies:
...
def load_reasoning_token_set(json_path = "./data/crucial_token_final_version.json"):
    with open(json_path, "r", encoding="utf-8") as f:
        token_list = json.load(f)
    print("Loaded reasoning token set from:", json_path)
...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        with self.update_sampling_params(logprobs=10,**prompts.meta_info):
            ...
            reasoning_token_set = load_reasoning_token_set("./data/crucial_token_final_version.json")
