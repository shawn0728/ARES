...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'global_id' in example:
            example['global_id'] = example.get("global_id", f"example-{index:06d}")
        return example

class FiltereRLHFdDataset(torch.utils.data.Dataset):
    def __init__(self, base_dataset, skip_gid_set):
        self.base_dataset = base_dataset
        self.skip_gid_set = skip_gid_set
        self.valid_indices = [
            idx for idx in range(len(base_dataset))
            if base_dataset.dataset[idx].get("global_id", f"example-{idx:06d}") not in skip_gid_set
        ]
